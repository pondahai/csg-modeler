...
這件事會影響你對速度和容量的直覺，值得花一分鐘理解。

軟體裡其實有三種東西，但**只有第一種會被存下來**：

1. **物件樹** — 就是左邊面板那份清單：有哪些方塊圓柱、各自多大、在哪裡、
   是實體還是孔洞、誰跟誰組合。這是**存檔和自動暫存唯一存的東西**。
2. **BSP 樹** — 挖洞的時候臨時搭起來的鷹架，算完就丟。從來不存。
3. **算出來的三角形** — 你畫面上看到的成品。也是算完就在記憶體裡，關掉就沒。

一個方塊只要記「20×10×5、放在這裡、是孔洞」，十幾個字就夠了 —— 這就是為什麼
存檔只有幾 KB。**匯入的 STL 是唯一的例外**：它沒有公式可以描述，只能一個一個
三角形記下來，所以一個五萬面的模型，光它自己就佔掉幾 MB。

**代價是每次打開都要重算一遍**，所以大模型開檔要等。好處是**你隨時可以把那個
圓柱孔從 5mm 改成 8mm** —— 如果存的是算完的三角形，那些參數早就不見了，
組合也拆不開了。這是這個工具的核心，不會為了開檔快而改掉。

（等待的時候畫面不會是空的：軟體會先把零件原本的樣子用骨架畫出來，
算好一個就換上真的一個。）
...
- **暫存** — 自動暫存用掉多少空間（上限約 5MB，快滿會提醒你手動存檔；
  真的滿了也不會全部丟掉，見下方「會踩到的地方」）
- **重算** — 上次重新計算花了多久
- **存檔** — 上次按存檔是多久以前，還沒存過會用紅字提醒

油表下面固定有一行寫**暫存放在哪裡**，因為那會隨環境改變：

- **大倉庫**（網頁版）—— 瀏覽器允許的話會用 IndexedDB，容量是 GB 等級，
  基本上用不完
- **小格子**（從隨身碟開啟時多半是這個）—— 約 5MB，塞不下時只留清單
- **不能暫存**（無痕視窗、某些瀏覽器設定）—— 那一根變成灰色斜紋、數字寫「無」，
  下面轉成紅字提醒。這時候「存檔」那一行就是你唯一的救生索

**三種情況的操作方式完全一樣**，只有容量不同，不必記兩套。
...
**「畫面上做得動」和「關掉還回得來」是兩回事。** 上面那些面數門檻管的是前者；
自動暫存另有一道約 5MB 的限制，是瀏覽器給的，改不掉。一般建模（方塊圓柱那些）
只佔幾百個字，永遠碰不到；**只有匯入的大模型會撐爆它**。

撐爆的時候軟體會**退一步，而不是整包放棄**：把匯入模型的三角形拿掉、其他照常存。
所以：

- **當下畫面完全正常**，你的模型還在，可以繼續做。變的只有油表下面那行字：
  「暫存不夠，匯入的模型沒存到（其他都在），請手動存檔！」
- **看到那行字就按一下存檔**（存成 JSON），那份檔案是完整的，什麼都不會少。
- **如果沒理它就關掉瀏覽器**：下次打開，你手工排好的方塊、圓柱、孔洞、對齊、
  組合關係**全部都在**；匯入的那個模型變成一個**空框**，物件樹上標著
  「⚠ 名字（要重匯）」，開檔時也會列出是哪幾個。把原檔重新匯入就接回去了。

會這樣取捨的理由很簡單：**佔掉空間的那個東西，剛好是最容易補回來的那個** ——
STL 檔還在你電腦裡，重匯一次是幾秒鐘；你排了半小時的那些，沒存到就是真的沒了。
...
尺標、貼到斜面（現在的表面吸附只會讓物件坐上去，不會跟著斜面轉向）。

**不會做可移動的工作平面。** 匯出 SVG 取的是「模型跟工作平面相交的那一層」，
要切哪裡就把模型上下搬 —— 平面也能動的話，每次都要先想「這次該搬模型還是搬
平面」。它真正好用的場合（把東西擺到斜面上）由「貼到斜面」解決，更直接。
